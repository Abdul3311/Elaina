...
export async function groupsUpdate(groupsUpdate) {
    ...
    for (const groupUpdate of groupsUpdate) {
        ...
        if (groupUpdate.desc) text = (chats.sDesc || this.sDesc || conn.sDesc || '```Deskripsi Group Telah Diubah Menjadi```\n@desc').replace('@desc', groupUpdate.desc)
        if (groupUpdate.subject) text = (chats.sSubject || this.sSubject || conn.sSubject || '```Nama Grup Telah Diubah Menjadi```\n@subject').replace('@subject', groupUpdate.subject)
        if (groupUpdate.icon) text = (chats.sIcon || this.sIcon || conn.sIcon || '```Foto Grup Telah Diubah Menjadi```').replace('@icon', groupUpdate.icon)
        if (groupUpdate.revoke) text = (chats.sRevoke || this.sRevoke || conn.sRevoke || '```Link Group Telah Diubah Menjadi```\n@revoke').replace('@revoke', groupUpdate.revoke)
        if (!text) continue
...
title: 'DONT FORGET TO VISIT MY API',
body: bottime, 
thumbnail: fs.readFileSync("./thumbnail.jpg"),
mediaType:tautanwa,
mediaUrl: "tautanwa",
